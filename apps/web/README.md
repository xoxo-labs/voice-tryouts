...
The experiments app. Each experiment is a self-contained route under
...
### Voice prompt input (AI Elements)

`/experiments/voice-prompt-input` — a Vercel AI Elements `PromptInput` with its
microphone button wired through `useVoiceInput` from
`@xoxo-labs/realtime-transcribe/react`, replacing the built-in Web Speech
recognition with real API transcription (cross-browser, model choice, delay
control, and `ws-preroll` so words spoken during connection are not lost).
There is no chat backend — the input itself is the experiment; submit echoes
the text on the page.

## Getting started

This app lives in a pnpm + Turborepo monorepo — run everything from the repo
root:
